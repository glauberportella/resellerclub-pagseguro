...
<head>
	<meta charset="utf-8">
	<title>Desenvolve4Web - Pagamento</title>
</head>
<body>

<?php
	// Prepares the database for save transactions
	\ResellerClubPagseguro\Config::prepareDatabase();

	// Reseller club key (configured on \ResellerClubPagSeguro\Config)
	...
	if (verifyChecksum($paymentTypeId, $transId, $userId, $userType, $transactionType, $invoiceIds, $debitNoteIds, $description, $sellingCurrencyAmount, $accountingCurrencyAmount, $key, $checksum))
	{
		...
		// the transaction data from ResellerClub
		foreach ($resellerPaymentData as $key => $value) {
			$transactionData[$key] = $value;
		}
		// the customer data from ResellerClub
		foreach ($customerData as $key => $value) {
			$transactionData[$key] = $value;
		}
		// IMPORTANT the transactionData array must keep the order of keys as the database table columns order
		$payment->saveTransaction(\ResellerClubPagseguro\Config::databaseConnection(), $transactionData);
